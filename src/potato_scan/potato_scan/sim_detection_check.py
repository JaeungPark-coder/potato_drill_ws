"""Run the eye detector against the Isaac Sim potato, with no Isaac Sim.
    ...
    python -m potato_scan.sim_detection_check --seeds 30 --noise-mm 0.2 --curvature-min 0.02
...
WHAT IT SETTLED, 2026-09-15 (30 potatoes, defaults)
...
DEFAULTS = dict(knn=30, curvature_min=0.015, shape_index_max=0.35,
...
def detect(geometry, noise, params, voxel_size=VOXEL_SIZE, sample_seed=0):
    ...
    _, kappa, _ = describe_surface(points, geometry.center, knn=params['knn'])
    ...
    return points, kappa, candidates, result


def report_one(seed, noise, params, with_eyes=True):
    geometry = generate(POTATO_CENTER, seed=seed, with_eyes=with_eyes)
    points, kappa, candidates, result = detect(geometry, noise, params)
    q = np.percentile(kappa, [50, 90, 99])
    ...
    print(f'  kappa p50/p90/p99: {q[0]:.4f} / {q[1]:.4f} / {q[2]:.4f}   '
          f"(curvature_min {params['curvature_min']}"
          f"{' -- BELOW p90, expect spurious' if params['curvature_min'] < q[1] else ''})")
    print()
    ...
    if candidates:
        ...
        for d, c in enumerate(candidates):
            ...
            print(f"    detection {d} [{tag:8s}] diam {c['diameter'] * 1000:4.1f}mm "
                  f"pts {c['points']:3d} S {c['shape_index']:.2f} "
...
def report_many(seeds, noise, params, with_eyes=True):
    ...
    for seed in seeds:
        geometry = generate(POTATO_CENTER, seed=seed, with_eyes=with_eyes)
        _, kappa, _, result = detect(geometry, noise, params)
        s = summarize(result)
        found += s['found']
        truth += s['of']
        spurious += s['spurious']
    ...
    line = (f'{len(seeds)} potatoes, {label}, {noise * 1000:.2f}mm noise, '
            f"curvature_min {params['curvature_min']}: kappa p90 ~{np.mean(kappa90):.4f} | "
            f'found {found}/{truth}')
    if truth:
...
def main(argv=None):
    ...
    for name in ('curvature_min', 'shape_index_max', 'cluster_eps', 'max_diameter'):
    ...
    for name in ('knn', 'curvature_min', 'shape_index_max', 'cluster_eps',
                 'cluster_min_points', 'max_diameter'):
